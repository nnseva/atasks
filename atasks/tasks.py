...
def atask_queue(coro=None, name=None, namespace='default', **options):
    ...
    consumer instance among all instances which had this same name registered
    at the moment they called ``atasks.router.get_router(namespace).activate()``
    processes each published event - classic AMQP work-queue semantics, the
    right choice for specialized single-purpose consumer services (e.g. rating
    recalculation, notification generation) where an event must be handled
    exactly-once-per-publish (well, at-least-once - see the idempotency note
    in the README), never once per running instance.
...
def atask_broadcast(coro=None, name=None, namespace='default', **options):
    ...
    which had this same name registered at the moment it called
    ``atasks.router.get_router(namespace).activate()`` receives and processes
    its own independent copy of the event (as opposed to ``atask_queue``,
    where instances compete for a single delivery). This is the mode used by a
    fleet of ``ws_gateway`` instances, each of which needs every event to
    filter and relay to its own independently-held WebSocket connections.
